...
public class GainOrLosePoint : MonoBehaviour {
    ...
    public static async Task examineUpdateScoreAsync() {
        if (Time.realtimeSinceStartup - User.lastTime > 3) {
            ...
            try {
                ...
                print("User score befroe: " + User.score);
    ...
    public static async Task updateLeaderboardAsync() {
        ...
        var r = await User.client.WriteLeaderboardRecordAsync(User.session, leaderboardID, Convert.ToInt64(User.score));
        UnityMainThreadDispatcher.Instance().Enqueue(() => EventCenter.Instance.TriggerEventListener("UpdateTopRightScore"));
        print(r);
    }
